...
const email = `portfolio_tester_${Date.now()}@example.com`;
...
async function runE2E() {
    console.log("=== STARTING PORTFOLIO E2E VALIDATION ===");
    ...
    const prompt = `Create a modern, responsive frontend-only portfolio website for a Software Developer.

Use React.js + Tailwind CSS. Do not build a backend, database, authentication system, admin panel, or server-side functionality.

The generated website must include:
- Responsive Navbar with mobile menu
- Hero section with View Projects and Contact Me CTAs
- About section
- Skills section
- At least 3 realistic software projects
- Reusable SkillCard components where appropriate
...
- Meaningful safe button behavior
- No dead buttons
- No broken imports
...
Use React.js, Vite, Tailwind CSS, and JavaScript.
Generate a complete structured project including all required configuration and source files.`;
    ...
    try {
        ...
        if (!healthRes.data.includes('id=\"root\"') && !healthRes.data.includes('src=\"/src/main.jsx\"')) {
            throw new Error("HTML response is missing entry div or source file mount marker!");
        }
        console.log("-> Health Check verified structure of served preview HTML successfully!");
    } catch (err) {
    ...
    const zipPath = path.resolve(__dirname, "portfolio.zip");
    try {
    ...
    const extractDir = path.resolve(__dirname, "portfolio_extracted");
    ...
    try {
        ...
        console.log("\n=== ALL PORTFOLIO E2E VALIDATION STEPS PASSED SUCCESSFULLY! ===");
        
        // Final cleanup
        try {
            fs.rmSync(zipPath, { force: true });
            fs.rmSync(extractDir, { recursive: true, force: true });
            console.log("-> Cleaned up temporary portfolio build folders.");
        } catch (e) {}
