...
const { sendChatCompletion } = require("../../services/aiProviders/openRouterProvider");
...
class OpenRouterProvider extends AIProvider {
    ...
    /**
     * Initializes the OpenRouter provider with configuration.
     *
    ...
    async initialize(config) {
        ...
        this.config = {
            ...
            model: config.model || process.env.OPENROUTER_MODEL || "google/gemini-2.5-flash"
        };
    ...
    async chat(request) {
        ...
        process.env.OPENROUTER_API_KEY = this.config.apiKey;
        process.env.OPENROUTER_MODEL = request.model || this.config.model;
        ...
        try {
            ...
            const mappedErr = new Error(`OpenRouter chat completion failed: ${err.message}`);
    ...
    async stream(request) {
        const err = new Error("Method 'stream' is not supported by OpenRouter adapter.");
        err.code = providerInterfaceErrorCodes.PROVIDER_INTERFACE_UNSUPPORTED_OPERATION;
        throw err;
    }
    ...
    async countTokens(request) {
        const err = new Error("Method 'countTokens' is not supported by OpenRouter adapter.");
        err.code = providerInterfaceErrorCodes.PROVIDER_INTERFACE_UNSUPPORTED_OPERATION;
        throw err;
    }
